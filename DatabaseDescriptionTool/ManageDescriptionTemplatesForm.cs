...
public partial class ManageDescriptionTemplatesForm : Form
{
	private bool _anyChanges;
	private Rectangle _dragBoxFromMouseDown;
	private int _rowIndexFromMouseDown;
	private int _rowIndexOfItemUnderMouseToDrop;
	private readonly List<DescriptionTemplate> _copiedItems = new List<DescriptionTemplate>();
	private bool _cutActivated;

	public ManageDescriptionTemplatesForm()
	{
		InitializeComponent();
		FillList();
		SelectFirstItem();
	...
	private void FillList()
	{
		...
		for (int i = 0; i < ConfigHandler.DescriptionTemplates.Count; i++)
		{
			int index = dataGridView1.Rows.Add();
			DataGridViewRow row = dataGridView1.Rows[index];
			row.Cells["ItemName"].Value = ConfigHandler.DescriptionTemplates[i].Name;
			row.Cells["ItemName"].ToolTipText = ConfigHandler.DescriptionTemplates[i].Name;
		}
	...
	private void Save()
	{
		List<DescriptionTemplate> newList = new List<DescriptionTemplate>();
		...
		ConfigHandler.DescriptionTemplates = newList;
		_anyChanges = true;
	...
	private void Delete()
	{
		DialogResult result = MessageBox.Show("Delete selected Description Templates?", ConfigHandler.ApplicationName, MessageBoxButtons.YesNo, MessageBoxIcon.Information);

		if (result.ToString() == "Yes")
		{
			List<DescriptionTemplate> itemsToBeDeleted = new List<DescriptionTemplate>();
	...
	private void DoDelete(List<DescriptionTemplate> itemsToBeDeleted)
	{
		...
		for (int r = 0; r < dataGridView1.Rows.Count; r++)
		{
			...
			{
				if (dataGridView1.Rows[r].Cells["ItemName"].Value.ToString() == itemsToBeDeleted[i].Name)
				{
		...
		for (int i = 0; i < indexesToBeRemoved.Count; i++)
		{
			...
			ConfigHandler.DescriptionTemplates.RemoveAt(indexesToBeRemoved[i]);
		}
	...
	private void ImportToolStripMenuItem_Click(object sender, EventArgs e)
	{
		...
		if (result.ToString() == "OK")
		{
			...
			if (dataGridView1.Rows.Count > 0)
			{
				DialogResult overwrite = MessageBox.Show("All existing Description Templates will be replaced with the new ones.\r\n\r\nContinue?", ConfigHandler.ApplicationName, MessageBoxButtons.YesNo, MessageBoxIcon.Information);
			...
			if (doimport)
			{
				...
				ConfigHandler.DescriptionTemplates = ConfigHandler.XmlToDescriptionTemplates(xml);
				_anyChanges = true;
				ConfigHandler.SaveConfig();
	...
	private void ExportToolStripMenuItem_Click(object sender, EventArgs e)
	{
		...
		if (result.ToString() == "OK")
		{
			...
			string xml = ConfigHandler.GetDescriptionTemplateListToXml(ConfigHandler.DescriptionTemplates);
	...
	private void DoCopy()
	{
		...
		for (int i = 0; i < ConfigHandler.DescriptionTemplates.Count; i++)
		{
			...
			{
				if (dataGridView1.SelectedRows[r].Cells["ItemName"].Value.ToString() == ConfigHandler.DescriptionTemplates[i].Name)
				{
					_copiedItems.Add(ConfigHandler.DescriptionTemplates[i]);
				}
			}
		}
	}
	...
	{
		DialogResult overwrite = MessageBox.Show("Delete all Description Templates?", ConfigHandler.ApplicationName, MessageBoxButtons.YesNo, MessageBoxIcon.Information);

		if (overwrite.ToString() == "Yes")
		{
			ConfigHandler.DescriptionTemplates = null;
			_anyChanges = true;
			ConfigHandler.SaveConfig();
			FillList();
		}
